...
class DepthReader:
    ...
    def depth_to_ground_map(self, altitude: float, drone_n: float, drone_e: float,
                            fov_radius: float = 3.0) -> list:
        """
        Convert depth image to list of (n, e, height_offset) points.

        Projects depth pixels to ground NED coordinates relative to drone.
        This feeds into the SafetyGrid.

        Args:
            altitude: Drone height above ground
            drone_n, drone_e: Current drone NED position
            fov_radius: Approximate ground coverage radius at altitude

        Returns:
            List of (n, e, state, height_offset) tuples
        """
        points = []
        ...
        for r in range(grid.shape[0]):
            for c in range(grid.shape[1]):
                ...
                # Map grid cell to ground NED offset
                # Center cell = drone position
                frac_r = (r - grid.shape[0] / 2) / (grid.shape[0] / 2)
                frac_c = (c - grid.shape[1] / 2) / (grid.shape[1] / 2)
                offset_n = -frac_r * fov_radius  # Negative because image Y is down
                offset_e = frac_c * fov_radius

                n = drone_n + offset_n
                e = drone_e + offset_e

                # Height offset: how much shorter is the depth reading?
                height_offset = altitude - depth
                if height_offset < 0:
                    height_offset = 0
