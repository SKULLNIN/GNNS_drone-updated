...
class PIDController:
    ...
    def __init__(self, gains: PIDGains):
        ...
        self._d_filter_alpha = 0.3  # Low-pass filter on D-term
...
class FlightController:
    ...
    def __init__(self, config: FlightConfig = None):
        ...
        self._pos_filter = PositionFilter(alpha=0.4)
    ...
    def compute_waypoint_velocity(self, target_n: float, target_e: float,
        ...
        err_alt = target_alt - current_alt  # positive = need to go up

        # Filter position before PID (smooths VIO noise → prevents wobble)
        err_n = self._pos_filter.update('err_n', err_n)
        err_e = self._pos_filter.update('err_e', err_e)
        err_alt = self._pos_filter.update('err_alt', err_alt)
