...
class MAVLinkBridge:
        ...
        while self._running:
            try:
                ...
                # Handle STATUSTEXT from FC
                    ...
                    sev = severity_names[msg.severity] if msg.severity < 8 else "?"
                    logger.info(f"FC [{sev}]: {msg.text}")
    ...
    def arm(self, timeout: float = 5.0, retries: int = 1) -> bool:
        ...
        for attempt in range(retries):
            ...
            if attempt < retries - 1:
                logger.warning(f"Arm failed, retrying in 3s...")
                time.sleep(3)
        
        logger.error("Arm FAILED — check pre-arm messages in FC")
        return False
    ...
    def send_velocity_ned(self, vx: float, vy: float, vz: float,
        ...
        Centralized here so navigator and precision_landing don't duplicate.
